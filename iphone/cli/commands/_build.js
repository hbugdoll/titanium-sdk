...
class iOSBuilder extends Builder {
			...
			ioslib.detect({
				...
				cli.createHook('build.ios.config', function (callback) {
					callback(null, {
						flags: {
							'force-copy': {
								desc: `forces files to be copied instead of symlinked for ${'simulator'.cyan} builds only`
							},
							'hide-error-controller': {
								hidden: true
							},
							'launch-watch-app': {
								desc: `for ${'simulator'.cyan} builds, after installing an app with a watch extention, launch the watch app and the main app`
							},
							'launch-watch-app-only': {
								desc: `for ${'simulator'.cyan} builds, after installing an app with a watch extention, launch the watch app instead of the main app`
							},
							'sim-focus': {
								default: true,
								desc: 'focus the iOS Simulator'
							},
							xcode: {
								// DEPRECATED
								...
								callback: function (value) {
									if (value) {
										...
										logger.error('The generated Titanium Xcode project is too old.');
										logger.error('Please clean and rebuild the project.');
										process.exit(1);
									}
								},
								hidden: true
	 ...
	 * @returns {Object}
	 */
	configOptionDeployType(order) {
		return {
			abbr: 'D',
			desc: `the type of deployment; only used when target is ${'simulator'.cyan} or ${'device'.cyan}`,
			hint: 'type',
			order: order,
		...
		return {
			...
			prompt: function (callback) {
				...
				if (this.cli.argv.target === 'device') {
					...
					params.title = 'Which device do you want to install to?';
					params.promptLabel = 'Select an device by number or name';
					...
					params.title = 'Which simulator do you want to launch?';
					params.promptLabel = 'Select an simulator by number or name';
					params.formatters.option = function (opt, idx, num) {
			...
			validate: function (udid, callback) {
				...
				if (info.udids[udid]) {
					...
					callback(new Error(this.cli.argv.target === 'device' ? `Invalid iOS device "${udid}"` : `Invalid iOS simulator "${udid}"`));
				}
			}.bind(this),
	 ...
	 * @returns {Object}
	 */
	configOptionDeveloperName(order) {
		const cli = this.cli,
		...
		return {
			...
			desc: `the iOS Developer Certificate to use; required when target is ${'device'.cyan}`,
			hint: 'name',
			order: order,
			prompt: function (callback) {
				var developerCerts = {},
				...
				callback(fields.select({
					title: 'Which developer certificate would you like to use?',
					promptLabel: 'Select a certificate by number or name',
					formatters: {
						option: function (opt, idx, num) {
							var expires = moment(opt.after),
								day = expires.format('D'),
								hour = expires.format('h');
							let label = '  ' + num + appc.string.rpad(opt.fullname, maxDevCertLen + 1).cyan;
							if (opt.after) {
								label += ` (expires ${
									expires.format('MMM')
								} ${day.length === 1 ? ' ' : ''}${day}, ${
			...
			validate: function (value, callback) {
				...
				if (value) {
					...
					if (v.length > 1) {
						return callback(new Error('Unable to determine correct certificate from supplied value'));
					}
				}
				callback(new Error(`Invalid developer certificate "${value}"`));
			}
		};
	}
	 ...
	 * @returns {Object}
	 */
	configOptionDistributionName(order) {
		const cli = this.cli,
			iosInfo = this.iosInfo,
		...
		return {
			...
			desc: `the iOS Distribution Certificate to use; required when target is ${'dist-appstore'.cyan} or ${'dist-adhoc'.cyan}`,
			hint: 'name',
			order: order,
			prompt: function (callback) {
				...
				callback(fields.select({
					title: 'Which distribution certificate would you like to use?',
					promptLabel: 'Select a certificate by number or name',
					formatters: {
						option: function (opt, idx, num) {
							var expires = moment(opt.after),
								day = expires.format('D'),
								hour = expires.format('h');
							let label = '  ' + num + appc.string.rpad(opt.fullname, maxDistCertLen + 1).cyan;
							if (opt.after) {
								label += ` (expires ${
									expires.format('MMM')
								} ${day.length === 1 ? ' ' : ''}${day}, ${
			...
			validate: function (value, callback) {
				...
				callback(new Error(`Invalid distribution certificate "${value}"`));
			}
		};
	}
	 ...
	 * @returns {Object}
	 */
	configOptionDeviceFamily(order) {
		return {
			abbr: 'F',
			desc: 'the device family to build for',
			order: order,
	...
	configOptioniOSVersion(order) {
		...
		return {
			...
			callback: function (value) {
				try {
					if (value && _t.iosAllSdkVersions.indexOf(value) !== -1 && version.lt(value, _t.minSupportedIosSdk)) {
						logger.banner();
						logger.error(`The specified iOS SDK version "${value}" is not supported by Titanium ${_t.titaniumSdkVersion}\n`);
						if (_t.iosSdkVersions.length) {
							logger.log('Available supported iOS SDKs:');
			...
			prompt: function (callback) {
				callback(fields.select({
					title: 'Which iOS SDK version would you like to build with?',
					promptLabel: 'Select an iOS SDK version by number or name',
					margin: '',
					numbered: true,
					relistOnError: true,
					complete: true,
	 ...
	 * @returns {Object}
	 */
	configOptionKeychain() {
		return {
			abbr: 'K',
			desc: `path to the distribution keychain to use instead of the system default; only used when target is ${
				'device'.cyan
			}, ${
				'dist-appstore'.cyan
			}, or ${
			...
			validate: function (value, callback) {
				...
				if (value && !fs.existsSync(value)) {
					callback(new Error(`Unable to find keychain: ${value}`));
				} else {
					callback(null, value);
				}
			}
	 ...
	 * @returns {Object}
	 */
	configOptionLaunchBundleId() {
		return {
			desc: `after installing the app, launch an different app instead; only used when target is ${'simulator'.cyan}`,
			hint: 'id'
		};
	}
	...
	configOptionOutputDir(order) {
		...
		return {
			abbr: 'O',
			desc: `the output directory when using ${'dist-appstore'.cyan} or ${'dist-adhoc'.cyan}`,
			hint: 'dir',
			order: order,
			prompt: function (callback) {
				callback(fields.file({
					promptLabel: 'Where would you like the output IPA file saved?',
	...
	configOptionPPuuid(order) {
		...
		return {
			abbr: 'P',
			desc: `the provisioning profile uuid; required when target is ${
				'device'.cyan
			}, ${
				'dist-appstore'.cyan
			}, or ${
			...
			prompt: function (callback) {
				...
				if (target === 'device') {
					if (iosInfo.provisioning.development.length) {
						...
						if (pp.length) {
							provisioningProfiles['Available Development UUIDs:'] = pp;
						} else {
							if (certs.length > 0) {
								logger.error(`Unable to find any non-expired development provisioning profiles that match the app id "${
									appId
								}" and the "${
									certs[0].name
								}" certificate.\n`);
							} else {
								logger.error(`Unable to find any non-expired development provisioning profiles that match the app id "${appId}"\n.`);
							}
							logger.log(`You will need to log in to ${
						...
						logger.error('Unable to find any development provisioning profiles\n');
						logger.log(`You will need to log in to ${
							'https://developer.apple.com/account/resources/certificates/list'.cyan
						} with your Apple Developer account, then create, download, and install a profile.\n`);
						process.exit(1);
					...
					if (iosInfo.provisioning.distribution.length) {
						...
						if (pp.length) {
							provisioningProfiles['Available App Store Distribution UUIDs:'] = pp;
						} else {
							logger.error(`Unable to find any non-expired App Store distribution provisioning profiles that match the app id "${
								appId
							}".\n`);
							logger.log(`You will need to log in to ${
								'https://developer.apple.com/account/resources/certificates/list'.cyan
						...
						logger.error('Unable to find any App Store distribution provisioning profiles');
						logger.log(`You will need to log in to ${
							'https://developer.apple.com/account/resources/certificates/list'.cyan
						} with your Apple Developer account, then create, download, and install a profile.\n`);
						process.exit(1);
					...
					if (iosInfo.provisioning.adhoc.length || iosInfo.provisioning.enterprise.length) {
						...
						let valid = pp.length;
						if (pp.length) {
							provisioningProfiles['Available Ad Hoc UUIDs:'] = pp;
						}

						pp = prep(iosInfo.provisioning.enterprise);
						valid += pp.length;
						if (pp.length) {
							provisioningProfiles['Available Enterprise Ad Hoc UUIDs:'] = pp;
						}

						if (!valid) {
							logger.error(`Unable to find any non-expired Ad Hoc or Enterprise Ad Hoc provisioning profiles that match the app id "${appId}".\n`);
							logger.log(`You will need to log in to ${
								'https://developer.apple.com/account/resources/certificates/list'.cyan
							} with your Apple Developer account, then create, download, and install a profile.\n`);
							process.exit(1);
						...
						logger.error('Unable to find any Ad Hoc or Enterprise Ad Hoc provisioning profiles');
						logger.log(`You will need to log in to ${
							'https://developer.apple.com/account/resources/certificates/list'.cyan
						} with your Apple Developer account, then create, download, and install a profile.\n`);
						process.exit(1);
				...
				callback(fields.select({
					title: 'Which provisioning profile would you like to use?',
					promptLabel: 'Select a provisioning profile UUID by number or name',
					formatters: {
						option: function (opt, idx, num) {
							const expires = opt.expirationDate && moment(opt.expirationDate),
								day = expires && expires.format('D'),
								hour = expires && expires.format('h');
							let label = `  ${num}${String(opt.uuid).cyan} ${appc.string.rpad(opt.label, maxAppId + 1)}`;
			...
			validate: function (value, callback) {
				...
				if (value) {
					...
					if (!p) {
						return callback(new Error(`Invalid provisioning profile UUID "${value}"`));
					}
					if (p.managed) {
						return callback(new Error(`Specified provisioning profile UUID "${value}" is managed and not supported`));
					}
					if (p.expired) {
						return callback(new Error(`Specified provisioning profile UUID "${value}" is expired`));
					}
					...
					if (certs.length > 0 && intersection(p.certs, pems).length === 0) {
						return callback(new Error(`Specified provisioning profile UUID "${
							value
	 ...
	 * @returns {Object}
	 */
	configOptionWatchAppName(order) {
		return {
			desc: `when building an app with multiple watch app, the name of the watch app to launch; only used when target is ${'simulator'.cyan}`,
			hint: 'name',
			order: order
		};
	}
	 ...
	 * @returns {Object}
	 */
	configOptionWatchDeviceId(order) {
		const cli = this.cli,
		...
		return {
			abbr: 'W',
			desc: `the watch simulator UDID to launch when building an app with a watch app; only used when target is ${'simulator'.cyan}`,
			hint: 'udid',
			order: order,
			prompt: function (callback) {
				...
				// simulator specific settings
				params.title = 'Which simulator do you want to launch your app in?';
				params.promptLabel = 'Select an simulator by number or name';
				params.formatters.option = function (opt, idx, num) {
					return '  ' + num + appc.string.rpad(opt.name, maxName).cyan + '  ' + opt.udid.grey;
				};
			...
			validate: function (value, callback) {
				if (!cli.argv['build-only'] && cli.argv.target === 'simulator') {
					if (!value || value === true) {
						...
						return callback(new Error(`Invalid Watch Simulator UDID "${value}"`));
					}
				}
				callback(null, value);
			}
	...
	initTiappSettings() {
		...
		if (!this.config.get('app.skipAppIdValidation') && !tiapp.properties['ti.skipAppIdValidation']) {
			if (!/^([a-zA-Z_]{1}[a-zA-Z0-9_-]*(\.[a-zA-Z0-9_-]*)*)$/.test(tiapp.id)) {
				logger.error(`tiapp.xml contains an invalid app id "${tiapp.id}"`);
				logger.error('The app id must consist only of letters, numbers, dashes, and underscores.');
				logger.error('Note: iOS does not allow underscores.');
			...
			if (tiapp.id.indexOf('_') !== -1) {
				logger.error(`tiapp.xml contains an invalid app id "${tiapp.id}"`);
				logger.error('The app id must consist of letters, numbers, and dashes.');
		...
		if (this.blacklistDirectories.indexOf(tiapp.name.toLowerCase()) !== -1 || tiapp.name.toLowerCase() === 'frameworks') {
			logger.error('The app name conflicts with a reserved file.');
			logger.error('You must change the name of the app in the tiapp.xml.\n');
			process.exit(1);
		}
		...
		if (!/\.png$/.test(tiapp.icon)) {
			logger.error('Application icon must be a PNG formatted image.\n');
			process.exit(1);
		}
		...
		if (!/^dist-(appstore|adhoc)$/.test(this.target) && logServerPort && (typeof logServerPort !== 'number' || logServerPort < 1024 || logServerPort > 65535)) {
			logger.error('Invalid <log-server-port> found in the tiapp.xml');
			logger.error('Port must be a positive integer between 1024 and 65535\n');
			process.exit(1);
		}
		...
		if (cli.argv.$originalPlatform === 'ipad') {
			logger.warn('--platform ipad has been deprecated and will be removed in Titanium SDK 7.0.0');
			logger.warn('See https://jira-archive.titaniumsdk.com/TIMOB-24228 for more details');
		}

		// init the extensions
		tiapp.ios.extensions.forEach(function (ext) {
			if (!ext.projectPath) {
				logger.error('iOS extensions must have a "projectPath" attribute that points to a folder containing an Xcode project.\n');
				process.exit(1);
			}
			...
			if (!fs.existsSync(ext.projectPath)) {
				logger.error(`iOS extension "${projectName}" Xcode project not found: ${ext.projectPath}\n`);
				process.exit(1);
			}

			const projFile = path.join(ext.projectPath, 'project.pbxproj');
			if (!fs.existsSync(projFile)) {
				logger.error(`iOS extension "${projectName}" project missing Xcode project file: ${projFile}\n`);
				process.exit(1);
			}
			...
			ext.project.targets.forEach(function (t) {
				...
				if (targetInfo.isWatchAppV1Extension || targetInfo.isWatchAppV1) {
					logger.error('WatchOS1 app detected.');
					logger.error(`Titanium ${this.titaniumSdkVersion} does not support WatchOS1 apps.\n`);
					process.exit(1);
				}
							...
							if (ext.objs.PBXGroup[child.value]) {
								...
								if (plist.WKWatchKitApp) {
									...
									if (CFBundleIdentifier.indexOf(tiapp.id) !== 0) {
										logger.error(`iOS extension "${
											ext.projectName
										}" WatchKit App bundle identifier is "${
											plist.CFBundleIdentifier
										}", but must be prefixed with "${
											tiapp.id
										}".\n`);
										process.exit(1);
									}
									...
									if (CFBundleIdentifier.toLowerCase() === tiapp.id.toLowerCase()) {
										logger.error(`iOS extension "${
											ext.projectName
										}" WatchKit App bundle identifier must be different from the Titanium app's id "${
											tiapp.id
										}".\n`);
										process.exit(1);
									}
									...
									logger.error(`The "${
										...
										targetName
									}" target's Info.plist is missing the WKWatchKitApp property, yet the product type is of a watch: ${
										productType
									}\n`);
									process.exit(1);
								}
				...
				if (cli.argv.target !== 'simulator' && cli.argv.target !== 'macos') {
					...
					if (!tiappTargets[targetName].ppUUIDs || !tiappTargets[targetName].ppUUIDs[cli.argv.target]) {
						if (cli.argv['pp-uuid']) {
							...
							// logger.warn(`iOS extension "${projectName}" target "<${cli.argv.target}>" is missing the ${targetName} provisioning profile UUID in tiapp.xml.`);
							// logger.warn(`Using the iOS app provisioning profile UUID "${cli.argv['pp-uuid']}"`);
						} else {
							logger.error(`iOS extension "${
								projectName
							}" target "<${
								...
								targetName
							} provisioning profile UUID in tiapp.xml.`);
							logger.log();
					...
					if (!pp) {
						logger.error(`iOS extension "${
							...
							targetName
						}" has invalid provisioning profile UUID in tiapp.xml.`);
						logger.error(`Unable to find a valid provisioning profile matching the UUID "${ppuuid}".\n`);
						process.exit(1);
					}
					...
					if (ext.targetInfo.id && !(new RegExp('^' + pp.appId.replace(/\*/g, '.*') + '$')).test(ext.targetInfo.id)) { // eslint-disable-line security/detect-non-literal-regexp
						...
						logger.error(`The provisioning profile "${
							ppuuid
						}" is tied to the application identifier "${
							pp.appId
						}", however the extension's identifier is "${
							ext.targetInfo.id
						}".`);
						logger.log();
						...
						if (matches.length) {
							...
							matches.forEach(function (m) {
								const expires = m.expirationDate && moment(m.expirationDate),
									day = expires && expires.format('D'),
									hour = expires && expires.format('h');
								let label = `  ${String(m.uuid).cyan} ${appc.string.rpad(m.appId, max + 1)}`;
								if (m.expirationDate) {
									label += ` (expires ${
										expires.format('MMM')
									} ${day.length === 1 ? ' ' : ''}${day}, ${
			...
			if (tiappTargets.length) {
				if (tiappTargets.length === 1) {
					logger.error(`iOS extension "${projectName}" does not contain a target named "${tiappTargets.join(', ')}".\n`);
				} else {
					logger.error(`iOS extension "${projectName}" does not contain the following targets: "${tiappTargets.join(', ')}".\n`);
				}
				process.exit(1);
			}
				...
				fs.existsSync(dir) && fs.readdirSync(dir).forEach(function (filename) {
					...
					if (this.blacklistDirectories.indexOf(lcaseFilename) !== -1) {
						if (isDir) {
							...
							logger.error('You must rename this directory to something else.\n');
						} else {
							logger.error('Found blacklisted file in the Resources directory.');
							logger.error(`The file "${filename}" is a reserved file.`);
						...
						if (isDir) {
							...
							logger.warn('There is a good chance your app will be rejected by Apple.');
							logger.warn('It is highly recommended you rename this directory to something else.');
						} else {
							logger.warn('Found graylisted file in the Resources directory.');
							logger.warn(`The file "${filename}" is potentially a reserved file.`);
							logger.warn('There is a good chance your app will be rejected by Apple.');
			...
			series(this, [
				...
				function selectDevice(next) {
					...
					if ((cli.argv['launch-watch-app'] || cli.argv['launch-watch-app-only']) && !this.hasWatchAppV2orNewer) {
						logger.warn(`${
							cli.argv['launch-watch-app'] ? '--launch-watch-app' : '--launch-watch-app-only'
						} flag was set, however there are no iOS extensions containing a watch app.`);
						logger.warn('Disabling launch watch app flag');
	...
	async run(logger, config, cli, finished) {
		try {
			...
			if (!this.buildOnly) {
				...
				logger.info(`Finished building the application in ${delta.cyan}`);
			}
	...
	async initialize() {
		...
		if (color) {
			...
			if (c && (c.length === 3 || c.length === 6)) {
				...
				this.logger.warn(`Invalid default background color "${color}" in the <ios> section of the tiapp.xml`);
				if (defaultColor) {
					this.logger.warn(`Using default background color "#${defaultColor}"`);
				}
			}
		}
	}

	findProvisioningProfile(target, uuid) {
	...
	loginfo() {
		this.logger.debug(`Titanium SDK iOS directory: ${cyan(this.platformPath)}`);
		this.logger.info(`Deploy type: ${cyan(this.deployType)}`);
		this.logger.info(`Building for target: ${cyan(this.target)}`);
		this.logger.info(`Building using iOS SDK: ${cyan(version.format(this.iosSdkVersion, 2))}`);

		if (this.buildOnly) {
			this.logger.info('Performing build only');
		} else if (this.target === 'simulator') {
			this.logger.info(`Building for iOS Simulator: ${cyan(this.simHandle.name)}`);
			this.logger.debug(`UDID: ${cyan(this.simHandle.udid)}`);
			this.logger.debug(`Simulator type: ${cyan(this.simHandle.family)}`);
			this.logger.debug(`Simulator version: ${cyan(this.simHandle.version)}`);
		} else if (this.target === 'device') {
			this.logger.info(`Building for iOS device: ${cyan(this.deviceId)}`);
		} else if (this.target === 'macos') {
			this.logger.info('Building for maccatalyst');
		}

		this.logger.info(`Building for device family: ${cyan(this.deviceFamily)}`);
		this.logger.debug(`Setting Xcode target to ${cyan(this.xcodeTarget)}`);
		this.logger.debug(`Setting Xcode build OS to ${cyan(this.xcodeTargetOS)}`);
		this.logger.debug(`Xcode installation: ${cyan(this.xcodeEnv.path)}`);
		this.logger.debug(`iOS WWDR certificate: ${cyan(this.iosInfo.certs.wwdr ? 'installed' : 'not found')}`);

		if (this.target === 'device') {
			this.logger.info(`iOS Development Certificate: ${cyan(this.certDeveloperName)}`);
		} else if (/^dist-appstore|dist-adhoc$/.test(this.target)) {
			this.logger.info(`iOS Distribution Certificate: ${cyan(this.certDistributionName)}`);
		}
		this.logger.info(`Team ID: ${this.teamId ? cyan(this.teamId) : 'n/a'.grey}`);

		// validate the min-ios-ver from the tiapp.xml
		this.logger.info(`Minimum iOS version: ${cyan(version.format(this.minIosVer, 2, 3))}`);

		if (/^device|dist-appstore|dist-adhoc$/.test(this.target)) {
			if (this.keychain) {
				this.logger.info(`Using keychain: ${cyan(this.keychain)}`);
			} else {
				this.logger.info('Using default keychain');
			}
		}

		if (this.tiLogServerPort) {
			this.logger.info(`Logging enabled on port ${cyan(String(this.tiLogServerPort))}`);
		} else {
			this.logger.info('Logging disabled');
		}

		if (this.debugHost) {
			this.logger.info(`Debugging enabled via debug host: ${cyan(this.debugHost)}`);
		} else {
			this.logger.info('Debugging disabled');
		}

		if (this.profilerHost) {
			this.logger.info(`Profiler enabled via profiler host: ${cyan(this.profilerHost)}`);
		} else {
			this.logger.info('Profiler disabled');
		}

		if (this.symlinkFilesOnCopy) {
			this.logger.info('Set to symlink files instead of copying');
		} else {
			this.logger.info('Set to copy files instead of symlinking');
		}
	...
	checkIfNeedToRecompile() {
		...
		this.forceCleanBuild = function () {
			...
			if (this.cli.argv.force) {
				this.logger.info(`Forcing clean build: ${cyan('--force')} flag was set`);
				return true;
			}

			// check if the build manifest file was read
			if (!Object.keys(this.previousBuildManifest).length) {
				this.logger.info(`Forcing clean build: ${cyan(this.buildManifestFile)} does not exist`);
				return true;
			}

			// check the <sdk-version> from the tiapp.xml
			if (!appc.version.eq(this.tiapp['sdk-version'], manifest.sdkVersion)) {
				this.logger.info('Forcing rebuild: tiapp.xml Titanium SDK version changed since last build');
				this.logger.info(`  Was: ${cyan(manifest.sdkVersion)}`);
				this.logger.info(`  Now: ${cyan(this.tiapp['sdk-version'])}`);
				return true;
			}

			// check if the deployType changed
			if (this.deployType !== manifest.deployType) {
				this.logger.info('Forcing rebuild: deployType changed since last build');
				this.logger.info(`  Was: ${manifest.deployType}`);
				this.logger.info(`  Now: ${this.deployType}`);
				return true;
			}
			...
			if (fs.existsSync(this.xcodeProjectConfigFile)) {
				...
				if (versionMatch && !appc.version.eq(versionMatch[1], this.titaniumSdkVersion)) {
					this.logger.info('Forcing rebuild: Titanium SDK version in the project.xcconfig changed since last build');
					this.logger.info(`  Was: ${cyan(versionMatch[1])}`);
					this.logger.info(`  Now: ${cyan(this.titaniumSdkVersion)}`);
					return true;
				}
			}

			if (this.certDeveloperName !== manifest.developerName) {
				this.logger.info('Forcing rebuild: developerName changed since last build');
				this.logger.info(`  Was: ${manifest.developerName}`);
				this.logger.info(`  Now: ${this.certDeveloperName}`);
				return true;
			}
		...
		this.forceRebuild = this.forceCleanBuild || function () {
			...
			if (!fs.existsSync(this.xcodeAppDir)) {
				this.logger.info(`Forcing rebuild: ${cyan(this.xcodeAppDir)} does not exist`);
				return true;
			}

			// check if the --force-copy flag was set
			if (this.forceCopy !== manifest.forceCopy) {
				this.logger.info('Forcing rebuild: force copy flag changed since last build');
				this.logger.info(`  Was: ${cyan(manifest.forceCopy)}`);
				this.logger.info(`  Now: ${cyan(this.forceCopy)}`);
				return true;
			}

			// check if the target changed
			if (this.target !== manifest.target) {
				this.logger.info('Forcing rebuild: target changed since last build');
				this.logger.info(`  Was: ${cyan(manifest.target)}`);
				this.logger.info(`  Now: ${cyan(this.target)}`);
				return true;
			}

			if (this.target === 'dist-adhoc' || this.target === 'dist-appstore' || this.target === 'dist-macappstore') {
				this.logger.info('Forcing rebuild: distribution builds require \'xcodebuild\' to be run so that resources are copied into the archive');
				return true;
			}
			...
			if (fs.existsSync(this.xcodeProjectConfigFile)) {
				...
				if (idMatch && idMatch[1] !== this.tiapp.id) {
					this.logger.info('Forcing rebuild: app id changed since last build');
					this.logger.info(`  Was: ${cyan(idMatch[1])}`);
					this.logger.info(`  Now: ${cyan(this.tiapp.id)}`);
					return true;
				}
			}
			...
			if (manifest.iosSdkPath !== this.platformPath) {
				...
				this.logger.info(`  Now: ${cyan(this.platformPath)}`);
				return true;
			}

			// check if the iOS SDK has changed
			if (manifest.iosSdkVersion !== this.iosSdkVersion) {
				this.logger.info('Forcing rebuild: iOS SDK version changed since last build');
				this.logger.info(`  Was: ${cyan(manifest.iosSdkVersion)}`);
				this.logger.info(`  Now: ${cyan(this.iosSdkVersion)}`);
				return true;
			}

			// check if the device family has changed (i.e. was universal, now iphone)
			if (manifest.deviceFamily !== this.deviceFamily) {
				this.logger.info('Forcing rebuild: device family changed since last build');
				this.logger.info(`  Was: ${cyan(manifest.deviceFamily)}`);
				this.logger.info(`  Now: ${cyan(this.deviceFamily)}`);
				return true;
			}

			// check the git hashes are different
			if (!manifest.gitHash || manifest.gitHash !== ti.manifest.githash) {
				this.logger.info('Forcing rebuild: githash changed since last build');
				this.logger.info(`  Was: ${cyan(manifest.gitHash)}`);
				this.logger.info(`  Now: ${cyan(ti.manifest.githash)}`);
				return true;
			}

			// determine the modules hash and check if the modules hashes has changed
			if (this.modulesHash !== manifest.modulesHash) {
				this.logger.info('Forcing rebuild: modules hash changed since last build');
				this.logger.info(`  Was: ${cyan(manifest.modulesHash)}`);
				this.logger.info(`  Now: ${cyan(this.modulesHash)}`);
				return true;
			}

			// check if the native modules hashes has changed
			if (this.modulesNativeHash !== manifest.modulesNativeHash) {
				this.logger.info('Forcing rebuild: native modules hash changed since last build');
				this.logger.info(`  Was: ${manifest.modulesNativeHash}`);
				this.logger.info(`  Now: ${this.modulesNativeHash}`);
				return true;
			}

			// check if the provisioning profile has changed
			if (this.provisioningProfileUUID !== manifest.ppUuid) {
				this.logger.info('Forcing rebuild: provisioning profile changed since last build');
				this.logger.info(`  Was: ${manifest.ppUuid}`);
				this.logger.info(`  Now: ${this.provisioningProfileUUID}`);
				return true;
			}

			// check if the use UserAutoLayout flag has changed
			if (this.useAutoLayout !== manifest.useAutoLayout) {
				this.logger.info('Forcing rebuild: use-autolayout flag changed since last build');
				this.logger.info(`  Was: ${manifest.useAutoLayout}`);
				this.logger.info(`  Now: ${this.useAutoLayout}`);
				return true;
			}

			// check if the use use-app-thinning flag has changed
			if (this.useAppThinning !== manifest.useAppThinning) {
				this.logger.info('Forcing rebuild: use-app-thinning flag changed since last build');
				this.logger.info(`  Was: ${manifest.useAppThinning}`);
				this.logger.info(`  Now: ${this.useAppThinning}`);
				return true;
			}

			// check if the use use-new-build-system flag has changed
			if (this.useNewBuildSystem !== manifest.useNewBuildSystem) {
				this.logger.info('Forcing rebuild: use-new-build-system flag changed since last build');
				this.logger.info(`  Was: ${manifest.useNewBuildSystem}`);
				this.logger.info(`  Now: ${this.useNewBuildSystem}`);
				return true;
			}

			// check if the showErrorController flag has changed
			if (this.showErrorController !== manifest.showErrorController) {
				this.logger.info('Forcing rebuild: showErrorController flag changed since last build');
				this.logger.info(`  Was: ${manifest.showErrorController}`);
				this.logger.info(`  Now: ${this.showErrorController}`);
				return true;
			}

			if (this.simOnlyActiveArch !== manifest.simOnlyActiveArch) {
				this.logger.info('Forcing rebuild: simOnlyActiveArch flag changed since last build');
				this.logger.info(`  Was: ${manifest.simOnlyActiveArch}`);
				this.logger.info(`  Now: ${this.simOnlyActiveArch}`);
				return true;
			}
			...
			if (changed) {
				this.logger.info('Forcing rebuild: tiapp.xml ${} changed since last build', tiappSettings[changed]);
				this.logger.info(`  Was: ${cyan(manifest[changed])}`);
				this.logger.info(`  Now: ${cyan(this.tiapp[changed])}`);
				return true;
			}
	...
	async initBuildDir() {
		this.logger.info('Initializing the build directory');

		const buildDirExists = await fs.exists(this.buildDir);

		if (this.forceCleanBuild && buildDirExists) {
			this.logger.debug(`Recreating ${cyan(this.buildDir)}`);
			await fs.emptyDir(this.buildDir);
		} else if (!buildDirExists) {
			this.logger.debug(`Creating ${cyan(this.buildDir)}`);
	...
	createXcodeProject(next) {
		...
		if (appc.version.lt(this.xcodeEnv.version, '7.0.0')) {
			this.logger.info(`LaunchScreen.storyboard is not supported with Xcode ${
				this.xcodeEnv.version
			}, removing from Xcode project`);
		}
		...
		this.logger.trace('Removing pre-compile phase');
		...
		const projectUuid = xcodeProject.hash.project.rootObject,
			...
			resourcesBuildPhase = xobjs.PBXResourcesBuildPhase[xobjs.PBXNativeTarget[mainTargetUuid].buildPhases.filter(function (phase) {
				return xobjs.PBXResourcesBuildPhase[phase.value];
			})[0].value],
		...
		if (this.nativeLibModules.length) {
			this.logger.trace(`Adding ${
				this.nativeLibModules.length
			} native module ${
				this.nativeLibModules.length === 1 ? 'library' : 'libraries'
			}`);
			...
			this.logger.trace('No native module libraries to add');
		}

		// add extensions and their targets to the project
		if (this.extensions.length) {
			this.logger.trace(`Adding ${this.extensions.length} iOS extension${this.extensions.length === 1 ? '' : 's'}`);
		...
		const hook = this.cli.createHook('build.ios.xcodeproject', this, function (xcodeProject, done) {
			...
			if (!fs.existsSync(dest) || contents !== fs.readFileSync(dest).toString()) {
				if (!this.forceRebuild) {
					this.logger.info('Forcing rebuild: Xcode project has changed since last build');
					this.forceRebuild = true;
				}
				this.logger.debug(`Writing ${dest.cyan}`);
	...
	_embedCapabilitiesAndWriteEntitlementsPlist(plist, dest, isExtension, next) {
		...
		const hook = this.cli.createHook(name, this, function (plist, dest, done) {
			...
			if (!fs.existsSync(dest) || contents !== fs.readFileSync(dest, 'utf-8').trim()) {
				if (!this.forceRebuild) {
					this.logger.info(`Forcing rebuild: ${dest.replace(this.projectDir + '/', '')} has changed since last build`);
					this.forceRebuild = true;
				}
				this.logger.debug(`Writing ${dest.cyan}`);
				fs.ensureDirSync(parent);
	...
	writeEntitlementsPlist(next) {
		this.logger.info('Creating Entitlements.plist');
		...
		if (fs.existsSync(entitlementsFile)) {
			this.logger.info(`Found custom entitlements: ${entitlementsFile.cyan}`);
	...
	writeInfoPlist() {
		this.logger.info('Creating Info.plist');
		...
		if (this.enableLaunchScreenStoryboard) {
			...
			if (appc.version.gte(this.xcodeEnv.version, '11.0.0')) {
				this.logger.warn('Launch images are deprecated by Xcode 11 and you will need to adopt a storyboard-based launch screen');
			}
		...
		if (fs.existsSync(customInfoPlistFile)) {
			this.logger.info('Copying custom Info.plist from project directory');
		...
		if (this.tiapp.iphone) {
			this.logger.error('The <iphone> section of the tiapp.xml has been removed in Titanium SDK 7.0.0 and later.');
			this.logger.log('Please use the <ios> section of the tiapp.xml to specify iOS-specific values instead:');
			this.logger.log();
		...
		if (!plist.NSAppTransportSecurity || typeof plist.NSAppTransportSecurity !== 'object') {
			this.logger.info('Disabling ATS');
			// disable ATS
		...
		if (Array.isArray(plist.UIAppFonts)) {
			plist.UIAppFonts.forEach(function (f) {
				if (!fontMap[f]) {
					this.logger.warn(`Info.plist references non-existent font: ${cyan(f)}`);
					fontMap[f] = 1;
				}
			}, this);
		}
		...
		if (plist.CFBundleIconFile) {
			this.logger.warn('Removing custom Info.plist "CFBundleIconFile" since we now use an asset catalog for app icons.');
			delete plist.CFBundleIconFile;
		}
		if (plist.CFBundleIconFiles) {
			this.logger.warn('Removing custom Info.plist "CFBundleIconFiles" since we now use an asset catalog for app icons.');
			delete plist.CFBundleIconFiles;
		}
		if (!Object.keys(i18nLaunchScreens).length) {
			...
			if (plist.UILaunchImages) {
				this.logger.warn('Removing custom Info.plist "UILaunchImages" since we now use an asset catalog for launch images.');
				delete plist.UILaunchImages;
			}
			if (plist['UILaunchImages~ipad']) {
				this.logger.warn('Removing custom Info.plist "UILaunchImages~ipad" since we now use an asset catalog for launch images.');
				delete plist['UILaunchImages~ipad'];
			}
		}
		if (plist.UILaunchImageFile) {
			this.logger.warn('Removing custom Info.plist "UILaunchImageFile" since we now use an asset catalog for launch images.');
			delete plist.UILaunchImageFile;
		}
		...
		if (!fs.existsSync(dest) || !prev || prev.size !== contents.length || prev.hash !== hash) {
			if (!this.forceRebuild) {
				this.logger.info('Forcing rebuild: Info.plist changed since last build');
				this.forceRebuild = true;
			}
			this.logger.debug(`Writing ${dest.cyan}`);
			fs.writeFileSync(dest, contents);
	...
	async writeMain() {
		this.logger.info('Creating main.m');
		...
		if (!fs.existsSync(dest) || contents !== fs.readFileSync(dest, 'utf8')) {
			if (!this.forceRebuild) {
				this.logger.info('Forcing rebuild: main.m has changed since last build');
				this.forceRebuild = true;
			}
			this.logger.debug(`Writing ${dest.cyan}`);
	...
	writeXcodeConfigFiles() {
		...
		if (!fs.existsSync(dest) || contents !== fs.readFileSync(dest).toString()) {
			if (!this.forceRebuild) {
				this.logger.info(`Forcing rebuild: ${path.basename(this.xcodeProjectConfigFile)} has changed since last build`);
				this.forceRebuild = true;
			}
			this.logger.debug(`Writing ${this.xcodeProjectConfigFile.cyan}`);
			fs.writeFileSync(dest, contents);
		} else {
			this.logger.trace(`No change, skipping ${this.xcodeProjectConfigFile.cyan}`);
		}
		this.unmarkBuildDirFile(dest);
		...
		if (!fs.existsSync(dest) || contents !== fs.readFileSync(dest).toString()) {
			if (!this.forceRebuild) {
				this.logger.info('Forcing rebuild: module.xcconfig has changed since last build');
				this.forceRebuild = true;
			}
			this.logger.debug(`Writing ${dest.cyan}`);
			fs.writeFileSync(dest, contents);
	...
	copyTitaniumiOSFiles() {
			...
			this.copyDirSync(path.join(this.platformPath, dir), path.join(this.buildDir, dir), {
				...
				beforeCopy: function (srcFile, destFile, srcStat) {
					...
					if (dir === 'Classes' && (filename === 'ApplicationRouting.m')) {
						this.logger.trace(`Skipping ${(dir + '/' + filename).cyan}, it'll be processed later`);
						return null;
					}
					...
					if (appFiles[filename]) {
						...
						if (!destExists || contents !== existingContent.toString()) {
							if (!this.forceRebuild) {
								this.logger.info(`Forcing rebuild: ${rel} has changed since last build`);
								this.forceRebuild = true;
							}
							this.logger.debug(`Writing ${destFile.cyan}`);
				...
				afterCopy: function (srcFile, destFile, srcStat, result) {
					if (!result) {
						this.logger.trace(`No change, skipping ${destFile.cyan}`);
					}
				}.bind(this)
			});
		...
		if (this.enableLaunchScreenStoryboard && this.defaultLaunchScreenStoryboard) {
			this.logger.info(`Installing default ${'LaunchScreen.storyboard'.cyan}`);
			copyAndReplaceFile.call(
				this,
	...
	copyExtensionFiles(next) {
		if (!this.extensions.length) {
			return next();
		}

		this.logger.info('Copying iOS extensions');
		...
		async.eachSeries(this.extensions, function (extension, next) {
			...
			this.logger.debug(`Copying ${extName.cyan}`);
					...
					if (path.basename(srcFile) === 'Info.plist' && !srcFile.includes('.framework')) {
						...
						if (infoPlist.WKWatchKitApp) {
							...
							if (!fs.existsSync(destFile) || contents !== fs.readFileSync(destFile).toString()) {
								if (!this.forceRebuild) {
									this.logger.info(`Forcing rebuild: iOS Extension "${extName}" has changed since last build`);
									this.forceRebuild = true;
								}
								if (changedCFBundleShortVersionString) {
									this.logger.warn(`WatchKit App's CFBundleShortVersionString "${
										origCFBundleShortVersionString
									}" does not match the app's CFBundleShortVersionString "${
										this.infoPlist.CFBundleShortVersionString
									}".`);
									this.logger.warn(`Setting the WatchKit App's CFBundleShortVersionString to "${
										this.infoPlist.CFBundleShortVersionString
									}"`);
								}
								if (changedCFBundleVersion) {
									this.logger.warn(`WatchKit App's CFBundleVersion "${
										origCFBundleVersion
									}" does not match the app's CFBundleVersion "${
										this.infoPlist.CFBundleVersion
									}".`);
									this.logger.warn(`Setting the WatchKit App's CFBundleVersion to "${
										this.infoPlist.CFBundleVersion
									}"`);
								}
								this.logger.debug(`Writing ${destFile.cyan}`);
			...
			function (cb) {
				...
				child.on('close', function (code) {
					if (code === 65) {
						...
						this.logger.debug(`Retrying to clean project (code ${code})`);
						lastErr = out;
						setTimeout(cb, 500);
					} else if (out.indexOf('INTERNAL ERROR') !== -1) {
						this.logger.debug('Retrying to clean project');
						lastErr = out;
						setTimeout(cb, 500);
					} else {
						done = true;
	...
	writeDebugProfilePlists() {
		this.logger.info('Creating debugger and profiler plists');
			...
			if (host) {
				...
				if (!exists || !prev || prev.size !== contents.length || prev.hash !== hash) {
					if (!this.forceRebuild && /device|dist-appstore|dist-macappstore|dist-adhoc/.test(this.target)) {
						this.logger.info(`Forcing rebuild: ${filename} changed since last build`);
						this.forceRebuild = true;
					}
					this.logger.debug(`Writing ${dest.cyan}`);
					fs.writeFileSync(dest, contents);
				...
				this.logger.debug(`Removing ${dest.cyan}`);
				fs.unlinkSync(dest);
			} else {
				this.logger.debug(`Skipping ${dest.cyan}`);
			}
	...
	async gatherResources() {
		...
		this.logger.info('Analyzing Resources directory');
		...
		this.logger.info('Analyzing module files');
		// detect ambiguous modules
		this.modules.forEach(module => {
			const filename = `${module.id}.js`;
			if (combined.has(filename)) {
				this.logger.error(`There is a project resource "${filename}" that conflicts with a native iOS module`);
				this.logger.error('Please rename the file, then rebuild\n');
				process.exit(1);
			}
		});
		...
		this.logger.info('Analyzing CommonJS modules');
		const commonJsResults = await Promise.all(this.commonJsModules.map(async module => {
			this.logger.info(`Analyzing CommonJS module: ${module.id}`);
		...
		this.logger.info('Analyzing localized launch images');
	...
	async copyCSSFiles(files) {
		this.logger.debug('Copying CSS files');
	 ...
	 */
	async processJSFiles(jsFilesMap) {
		this.logger.info('Processing JavaScript files');
	...
	async writeBootstrapJson(jsBootstrapFiles) {
		this.logger.info('Writing bootstrap json');
		...
		if (!fs.existsSync(bootstrapJsonAbsolutePath) || (bootstrapJsonString !== fs.readFileSync(bootstrapJsonAbsolutePath, 'utf8'))) {
			this.logger.debug(`Writing ${bootstrapJsonAbsolutePath.cyan}`);
			...
			this.logger.trace(`No change, skipping ${bootstrapJsonAbsolutePath.cyan}`);
		}
	...
	async writeAppProps() {
		this.logger.info('Writing app properties');
		...
		if (!fs.existsSync(appPropsFile) || contents !== fs.readFileSync(appPropsFile, 'utf8')) {
			this.logger.debug(`Writing ${appPropsFile.cyan}`);
			...
			this.logger.trace(`No change, skipping ${appPropsFile.cyan}`);
		}

		this.unmarkBuildDirFile(appPropsFile);
	}

	async writeEnvironmentVariables() {
		this.logger.debug('Writing ENV variables');
		...
		if (!fs.existsSync(envVarsFile) || contents !== fs.readFileSync(envVarsFile, 'utf8')) {
			this.logger.debug(`Writing ${envVarsFile.cyan}`);
			...
			this.logger.trace(`No change, skipping ${envVarsFile.cyan}`);
		}
	 ...
	 */
	async processAssets(imageAssets, launchImages, launchLogos, appIcons, resourcesToCopy) {
		this.logger.info('Creating asset catalog');
	...
	async generateSemanticColors() {
		...
		if (!(await fs.exists(colorsFile))) {
			...
			if (!(await fs.exists(colorsFile))) {
				this.logger.debug('Skipping colorset generation as "semantic.colors.json" file does not exist');
				return;
			}
		}
	 ...
	 * @param {Map.<string,FileInfo>} resourcesToCopy plain files to handle
	 */
	async createAssetImageSets(imageAssets, resourcesToCopy) {
		if (!this.useAppThinning) {
			this.logger.info('App thinning disabled, skipping asset image sets');
			return;
		}

		this.logger.info('Creating assets image set');
		const assetCatalog = path.join(this.buildDir, 'Assets.xcassets');
	 ...
	 * @param {Map.<string,FileInfo>} resourcesToCopy plain files to handle
	 */
	async createLaunchImageSet(launchImages, resourcesToCopy) {
		this.logger.info('Creating launch image set');
		...
		launchImages.forEach((info, filename) => {
			...
			if (!meta) {
				...
				this.logger.debug(`Unsupported launch image ${path.relative(this.projectDir, info.src).cyan}, skipping`);
				return;
			}

			// skip device specific launch images
			if (this.deviceFamily === 'iphone' && meta.idiom !== 'iphone') {
				this.logger.debug(`Skipping iPad launch image: ${path.relative(this.projectDir, info.src).cyan}`);
				return;
			}

			if (this.deviceFamily === 'ipad' && meta.idiom !== 'ipad') {
				this.logger.debug(`Skipping iPhone launch image: ${path.relative(this.projectDir, info.src).cyan}`);
				return;
			}
		...
		if (totalMissing) {
			...
			this.logger.warn(`Missing ${totalMissing === 1 ? 'a' : totalMissing} launch image${totalMissing === 1 ? '' : 's'}:`);
			Object.keys(missing).forEach(lang => {
				this.logger.warn('  ' + (lang === '_' ? 'Default' : 'i18n/' + lang));
				Object.keys(missing[lang]).forEach(filename => {
					const meta = lookup[filename];
					this.logger.warn(`    ${filename} - Used for ${
						meta.idiom === 'ipad' ? 'iPad' : 'iPhone'
					} - dimensions: ${meta.width}x${meta.height}, orientation: ${meta.orientation}`);
				});
			});
		}
	...
	async createAppIconSet(appIcons, launchLogos, resourcesToCopy) {
		...
		appIcons.forEach((info, filename) => {
			...
			if (pngInfo.width !== pngInfo.height) {
				...
				} because dimensions (${pngInfo.width}x${pngInfo.height}) are not equal`);
				return;
			}
			...
			if (pngInfo.width !== w) { // we enforce it's square above, so no need to check height too
				...
				this.logger.warn(`Expected app icon ${
					info.src.replace(this.projectDir + '/', '')
				} to be ${w}x${h}, but was ${pngInfo.width}x${pngInfo.height}, skipping`);
				return;
			}
			...
			if (!flatten) {
				this.logger.debug(`Found valid app icon ${
					info.src.replace(this.projectDir + '/', '').cyan
				} (${pngInfo.width}x${pngInfo.height})`);
		...
		if (missingIcons.length !== 0 && defaultIcon && defaultIconChanged && defaultIconHasAlpha && osName !== 'maccatalyst') {
			...
			this.logger.warn('You may create an image named "DefaultIcon-ios.png" that does not have an alpha channel in the root of your project');
		...
		if (!this.forceRebuild) {
			this.logger.info(`Forcing rebuild: ${
				defaultIcon.replace(this.projectDir + '/', '')
	 ...
	 * @returns {Promise<void>}
	 */
	flattenIcon(icon, flattenedDefaultIconDest) {
		return new Promise((resolve, reject) => {
			this.logger.debug(`Stripping alpha channel: ${icon.src.cyan} => ${icon.dest.cyan}`);
			const _t = this;
	 ...
	 */
	async processLaunchLogos(launchLogos, resourcesToCopy, defaultIcon, defaultIconChanged) {
		if (!this.enableLaunchScreenStoryboard || !this.defaultLaunchScreenStoryboard) {
			return [];
		}

		this.logger.info('Creating launch logo image set');
		...
		if (missingCount !== 0) {
			...
			if (launchLogo) {
				...
				if (changed) {
					...
					if (launchLogoInfo.width !== 1024 || launchLogoInfo.height !== 1024) {
						this.logger.warn(`Found LaunchLogo.png that is ${launchLogoInfo.width}x${launchLogoInfo.height}, however the size must be 1024x1024`);
						launchLogo = null;
					}
				}
			} else {
			...
			Object.keys(lookup).forEach(name => {
				...
				if (!logged) {
					logged = true;
					this.logger.info(
						`Missing ${missingCount} launch logo${
							missingCount === 1 ? '' : 's'
						}, generating missing launch logo from ${
		...
		if (!this.forceRebuild) {
			this.logger.info('Forcing rebuild: launch logos changed since last build');
			this.forceRebuild = true;
		}

		if (!this.buildOnly && (this.target === 'device' || this.target === 'simulator')) {
			this.logger.warn(`If this app has been previously installed on this ${this.target}, you may need restart it to see the latest launch logo`);
			this.logger.warn('iOS renders and caches the launch screen to a PNG image that seems to only be invalidated by restarting iOS');
		}
	 ...
	 */
	async copyDefaultLaunchLogos(assetCatalogDir, resourcesToCopy) {
		this.logger.warn('No DefaultIcon.png found, copying default Titanium LaunchLogo images');
	 ...
	 * @returns {Promise<void>}
	 */
	async copyUnmodifiedResources(resourcesToCopy) {
	...
	encryptJSFiles(next) {
		...
		if (!this.encryptJS || !this.jsFilesToEncrypt.length) {
			...
			this.logger.debug('Using default application routing');
		...
		const titaniumPrepHook = this.cli.createHook('build.ios.titaniumprep', this, function (exe, args, opts, done) {
			...
			this.jsFilesToEncrypt.forEach(file => this.logger.debug(`Preparing ${file.cyan}`));
			...
			async.whilst(
				...
				function (cb) {
					this.logger.debug(`Running ${(exe + ' "' + args.slice(0, -1).join('" "') + '"').cyan}`);
						...
						if (out.indexOf('initWithObjectsAndKeys') !== -1) {
							...
							if (!destExists || contents !== existingContent) {
								if (!this.forceRebuild) {
									...
									this.forceRebuild = true;
									this.logger.info(`Forcing rebuild: ${
										dest.replace(this.buildDir + '/', '').cyan
									} changed since last build`);
								}

								this.logger.debug(`Writing application routing source file: ${dest.cyan}`);
								fs.writeFileSync(dest, contents);
	...
	writeI18NFiles() {
		this.logger.info('Writing i18n files');
		...
		function add(obj, dest, map) {
			if (obj) {
				const rel = dest.replace(this.xcodeAppDir + '/', ''),
					contents = header + Object.keys(obj).map(function (name) {
						return '"' + (map && map[name] || name).replace(/\\"/g, '"').replace(/"/g, '\\"')
							+ '" = "' + ('' + obj[name]).replace(/${}/g, '%@').replace(/\\"/g, '"').replace(/"/g, '\\"') + '";';
					}).join('\n');
				...
				if (!fs.existsSync(dest) || contents !== fs.readFileSync(dest).toString()) {
					if (!this.forceRebuild && /device|dist-appstore|dist-macappstore|dist-adhoc/.test(this.target)) {
						this.logger.info(`Forcing rebuild: ${rel} changed since last build`);
						this.forceRebuild = true;
					}
					this.logger.debug(`Writing ${dest.cyan}`);
					fs.writeFileSync(dest, contents);
		...
		if (keys.length) {
			...
			this.logger.debug('No i18n files to process');
		}
	}
	...
	processTiSymbols() {
		...
		this.logger.debug('Processing Titanium namespace resources');
		Object.keys(namespaces).forEach(function (ns) {
	...
	removeFiles(next) {
		...
		const hook = this.cli.createHook('build.ios.removeFiles', this, function (done) {
			Object.keys(this.buildDirFiles).forEach(function (file) {
				try {
					this.logger.debug(`Removing ${file.cyan}`);
					fs.unlinkSync(file);
				} catch (ex) {
					// ignore
				}
			...
			if (this.target === 'dist-appstore' || this.target === 'dist-adhoc') {
				this.logger.info('Removing invalid architectures from TitaniumKit.xcframework');
				...
				async.eachSeries([ 'x86_64', 'i386' ], function (architecture, next) {
					...
					this.logger.debug(`Running: ${(this.xcodeEnv.executables.lipo + ' ' + args.join(' ')).cyan}`);
	...
	optimizeFiles(next) {
		...
		this.logger.info('Optimizing .plist and .png files');
		...
		function add(arr, name, file) {
			...
			if (!prev || prev.hash !== curr.hash) {
				...
				logger.trace(`No change, skipping ${file.cyan}`);
			}
		}
		...
		parallel(this, [
			function (next) {
				async.each(plists, function (file, cb) {
					this.logger.debug(`Optimizing ${file.cyan}`);
			...
			function (next) {
				if (!fs.existsSync(this.xcodeEnv.executables.pngcrush)) {
					this.logger.warn('Unable to find pngcrush in Xcode directory, skipping image optimization');
					return next();
				}
				...
				async.eachLimit(pngs, 5, function (file, cb) {
					...
					appc.subprocess.run(this.xcodeEnv.executables.pngcrush, [ '-q', '-iphone', '-f', 0, file, output ], function (code) {
						if (code) {
							this.logger.error(`Failed to optimize ${file} (code ${code})`);
						} else if (fs.existsSync(output)) {
							fs.existsSync(file) && fs.unlinkSync(file);
							fs.renameSync(output, file);
						} else {
							this.logger.warn(`Unable to optimize ${file}; invalid png?`);
						}
						cb();
					}.bind(this));
				}.bind(this), next);
	...
	async invokeXcodeBuild(next) {
		if (!this.forceRebuild) {
			this.logger.info('Skipping xcodebuild');
			return next();
		}

		this.logger.info('Invoking xcodebuild');
		...
		const xcodebuildHook = this.cli.createHook('build.ios.xcodebuild', this, function (exe, args, opts, done) {
			const DEVELOPER_DIR = this.xcodeEnv.path
				...
				+ args.map(function (a) { return a.indexOf(' ') !== -1 ? '"' + a + '"' : a; })
					.join(' ');
			this.logger.debug(`Invoking: ${('DEVELOPER_DIR=' + DEVELOPER_DIR).cyan}`);
